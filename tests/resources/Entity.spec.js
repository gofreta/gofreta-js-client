...
const Entity      = require('@/resources/Entity');

/**
 * Entity API resource tests.
 *
 * @author Gani Georgiev <gani.georgiev@gmail.com>
 */
describe('Entity', function () {
    var adapter  = new mockAdapter(axios);
    var resource = new Entity(axios.create());
    ...
    describe('getList()', function () {
        it('Should correctly set request data', function (done) {
            ...
            var result      = resource.getList('test', 2, 15, queryParams);

            assert.instanceOf(result, Promise);
            result.then(function (response) {
                assert.equal(response.config.url, '/collections/test/entities');
    ...
    describe('getOne()', function () {
        it('Should correctly set request data', function (done) {
            var queryParams = {'query_test1': 1, 'query_test2': 2};
            var result      = resource.getOne('test', 'myid', queryParams);

            assert.instanceOf(result, Promise);
            result.then(function (response) {
                assert.equal(response.config.url, '/collections/test/entities/myid');
                assert.equal(response.config.method, 'get');
    ...
    describe('create()', function () {
        it('Should correctly set request data', function (done) {
            ...
            var queryParams = {'query_test1': 1, 'query_test2': 2};
            var result      = resource.create('test', bodyParams, queryParams);
    ...
    describe('update()', function () {
        it('Should correctly set request data', function (done) {
            var bodyParams  = {'body_test1': 1, 'body_test2': 2};
            var queryParams = {'query_test1': 1, 'query_test2': 2};
            var result      = resource.update('test', 'myid', bodyParams, queryParams);
    ...
    describe('delete()', function () {
        it('Should correctly set request data', function (done) {
            var bodyParams  = {'body_test1': 1, 'body_test2': 2};
            var queryParams = {'query_test1': 1, 'query_test2': 2};
            var result      = resource.delete('test', 'myid', bodyParams, queryParams);
